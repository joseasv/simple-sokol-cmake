// -----------------------------------------------------------------------------
// 18_LuzFoco.cpp (Spotlight / Linterna)
// -----------------------------------------------------------------------------
...
// Struct alineado para Spotlight (96 bytes total)
struct {
    HMM_Vec3 light_position;
    float light_cutOff; // 0-16
    HMM_Vec3 light_direction;
    float light_outerCutOff; // 16-32
    HMM_Vec3 light_ambient;
    float light_constant; // 32-48
    HMM_Vec3 light_diffuse;
    float light_linear; // 48-64
    HMM_Vec3 light_specular;
    float light_quadratic; // 64-80
    float mat_shininess;
    float _pad[3]; // 80-96
} fs_params;
...
void init(void)
{
    ...
    state.cam_dist = 5.0f; // Más cerca para ver el efecto linterna
    state.cam_yaw = 0.0f;
...
void frame(void)
{
    ...
    // La cámara mira al centro (0,0,0)
    ...
    // --- CONFIGURACIÓN SPOTLIGHT ---
    // La luz está en la cámara (Linterna)
    fs_params.light_position = camPos;
    fs_params.light_direction = camFront;

    // Ángulos:
    // cutOff: 12.5 grados -> cos(12.5)
    // outerCutOff: 17.5 grados -> cos(17.5) (para cuando actives bordes suaves)
    fs_params.light_cutOff = cosf(12.5f * (PI / 180.0f));
    fs_params.light_outerCutOff = cosf(17.5f * (PI / 180.0f));

    fs_params.light_ambient = HMM_V3(0.1f, 0.1f, 0.1f);
    fs_params.light_diffuse = HMM_V3(0.8f, 0.8f, 0.8f);
...
sapp_desc sokol_main(int argc, char* argv[])
{
    ...
    desc.width = 800;
    desc.height = 600;
    desc.window_title = "18 - Luz Foco (Spotlight)";
